...
//                                     bubbleFrame.size.height - (_bubblePaddingY*2) - (_cornerRadius*2));
    CGRect contentFrame = CGRectMake(bubbleFrame.origin.x + _bubblePaddingX,
                                     bubbleFrame.origin.y + _bubblePaddingY,
                                     bubbleFrame.size.width - (_bubblePaddingX*2),
                                     bubbleFrame.size.height - (_bubblePaddingY*2));
    return contentFrame;
}
...
//    _bubbleSize = CGSizeMake(textSize.width + (_bubblePaddingX*2) + (_cornerRadius*2), textSize.height + (_bubblePaddingY*2) + (_cornerRadius*2));
    _bubbleSize = CGSizeMake(textSize.width + (_bubblePaddingX*2), textSize.height + (_bubblePaddingY*2));
	...
	if (animated) {
        if (self.animation == NXPopTipAnimationFade) {
            self.alpha = 0;
            self.frame = finalFrame;
        }
        else if (self.animation == NXPopTipAnimationSlide) {
            self.alpha = 0.0;
            CGRect startFrame = finalFrame;
            startFrame.origin.y += 10;
            self.frame = startFrame;
        }
		else if (self.animation == NXPopTipAnimationPop) {
            self.frame = finalFrame;
            self.alpha = 0.5;
            ...
            [UIView animateWithDuration:0.15 animations:^{
                wself.transform = CGAffineTransformMakeScale(1.1f, 1.1f);
                wself.alpha = 1.0;
            } completion:^(BOOL finished) {
                [wself popAnimationDidStop];
            }];
        }
...
- (void)autoDismissAnimated:(BOOL)animated atTimeInterval:(NSTimeInterval)timeInterval {
    ...
    __block int xTimeInterval = (int)timeInterval;
    __weak typeof(self) wself = self;
    ...
    dispatch_source_set_event_handler(_timer, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (xTimeInterval >= 0) {
                NSLog(@"xTimeInterval:%lu", (unsigned long)xTimeInterval);
            } else {
                dispatch_source_cancel(self->_timer);
                [wself dismissAnimated:animated];
                [wself notifyDelegatePopTipViewWasDismissedByUser];
            }
            xTimeInterval --;
        });
    });
...
- (void)notifyDelegatePopTipViewWasDismissedByUser {
	__strong id<NXPopTipViewDelegate> delegate = self.delegate;
...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:frame])) {
        ...
        self.hasShadow = YES;
        self.animation = NXPopTipAnimationSlide;
